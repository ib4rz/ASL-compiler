...
void SemErrors::incompatibleArgument(antlr4::tree::TerminalNode *node) {
  ErrorInfo error(node->getSymbol()->getLine(), node->getSymbol()->getCharPositionInLine(), "Incompatible argument types in function '" + node->getSymbol()->getText() + "'.");
  ErrorList.push_back(error);
}
...
void SemErrors::incompatibleOperator(antlr4::tree::TerminalNode *node) {
  ErrorInfo error(node->getSymbol()->getLine(), node->getSymbol()->getCharPositionInLine(), "Operator '" + node->getSymbol()->getText() + "' with incompatible types.");
  ErrorList.push_back(error);
}
...
void SemErrors::nonIntegerBucleControl(antlr4::tree::TerminalNode *node) {
  ErrorInfo error(node->getSymbol()->getLine(), node->getSymbol()->getCharPositionInLine(), "Integer type required in '" + node->getSymbol()->getText() + "' control variable.");
  ErrorList.push_back(error);
}
...
void SemErrors::numberOfArguments(antlr4::tree::TerminalNode *node) {
  ErrorInfo error(node->getSymbol()->getLine(), node->getSymbol()->getCharPositionInLine(), "Incorrect number of arguments in function '" + node->getSymbol()->getText() + "'.");
  ErrorList.push_back(error);
}

void SemErrors::incorrectNumber(antlr4::tree::TerminalNode *node) {
  ErrorInfo error(node->getSymbol()->getLine(), node->getSymbol()->getCharPositionInLine(), "Incorrect number of '" + node->getSymbol()->getText() + "' expressions.");
  ErrorList.push_back(error);
}
