...
antlrcpp::Any SymbolsVisitor::visitType(AslParser::TypeContext *ctx) {
  ...
  else if (ctx->array_decl()) {  // Array type
    visit(ctx->array_decl());
    putTypeDecor(ctx,getTypeDecor(ctx->array_decl()));
  }
  else {  // Pair type
    visit(ctx->pair_type());
    putTypeDecor(ctx,getTypeDecor(ctx->pair_type()));
  }
  DEBUG_EXIT();
  return 0;
}
...
antlrcpp::Any SymbolsVisitor::visitPair_type(AslParser::Pair_typeContext *ctx) {
  DEBUG_ENTER();
  visit(ctx->basic_type(0));
  visit(ctx->basic_type(1));
  TypesMgr::TypeId t1 = getTypeDecor(ctx->basic_type(0));
  TypesMgr::TypeId t2 = getTypeDecor(ctx->basic_type(1));
  TypesMgr::TypeId t = Types.createPairTy(t1,t2);
  putTypeDecor(ctx,t);
  DEBUG_EXIT();
  return 0;
}
