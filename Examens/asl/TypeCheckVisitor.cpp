...
antlrcpp::Any TypeCheckVisitor::visitForStmt(AslParser::ForStmtContext *ctx) {
  DEBUG_ENTER();
  for (auto ctxExpr : ctx->expr()) visit(ctxExpr);
  TypesMgr::TypeId t = getTypeDecor(ctx->expr(0));
  if (!Types.isErrorTy(t) && !Types.isIntegerTy(t))
    Errors.nonIntegerBucleControl(ctx->FOR());
  if (!((ctx->expr().size()-1 > 0) && (ctx->expr().size()-1 < 4)))
    Errors.incorrectNumber(ctx->INRANGE());
  for (size_t i = 1; i < ctx->expr().size(); ++i) {
    t = getTypeDecor(ctx->expr(i));
    if (!Types.isErrorTy(t) && !Types.isIntegerTy(t))
      Errors.nonIntegerBucleControl(ctx->INRANGE());
  }
  visit(ctx->statements());
  DEBUG_EXIT();
  return 0;
...
antlrcpp::Any TypeCheckVisitor::visitLeft_expr(AslParser::Left_exprContext *ctx) {
  ...
  if (ctx->expr()) {
    ...
    t1 = Types.getFirstPairType(t1);
  else if (ctx->SECOND())
    t1 = Types.getSecondPairType(t1);
...
antlrcpp::Any TypeCheckVisitor::visitFilter(AslParser::FilterContext *ctx) { 
  ...
  if (Types.isArrayTy(t1)) {
    TypesMgr::TypeId taux = Types.createArrayTy(Types.getArraySize(t1),Types.createBooleanTy());
    if (!Types.equalTypes(t2,taux))
      Errors.incompatibleOperator(ctx->FILTER());
    else {
      if (!(Types.isFunctionTy(t3) && Types.equalTypes(Types.getFuncReturnType(t3),Types.createBooleanTy())) ||
         (!(Types.equalTypes(Types.getParameterType(t3,0),Types.getArrayElemType(t1)) && Types.getNumOfParameters(t3) == 1)))
          Errors.incompatibleOperator(ctx->FILTER());
    }
  }
  else
...
antlrcpp::Any TypeCheckVisitor::visitPairExpr(AslParser::PairExprContext *ctx) {
  DEBUG_ENTER();
  visit(ctx->ident());
  TypesMgr::TypeId t1 = getTypeDecor(ctx->ident());
  TypesMgr::TypeId t = Types.createErrorTy();
  if (!Types.isErrorTy(t1)) {
    if (ctx->FIRST())
      t = Types.getFirstPairType(t1);
    else
      t = Types.getSecondPairType(t1);
  }
  bool b = getIsLValueDecor(ctx->ident());
  putTypeDecor(ctx, t);
  putIsLValueDecor(ctx, b);
...
antlrcpp::Any TypeCheckVisitor::visitArithmetic(AslParser::ArithmeticContext *ctx) {
  ...
  if (ctx->MOD()) {
    ...
    // Producte scalar
    if (ctx->MUL() && Types.isArrayTy(t1) && Types.isArrayTy(t2) &&
        Types.getArraySize(t1) == Types.getArraySize(t2)) {
      t1 = Types.getArrayElemType(t1); t2 = Types.getArrayElemType(t2);
      if (!(Types.isNumericTy(t1) && Types.isNumericTy(t2) && Types.equalTypes(t1,t2)))
        Errors.incompatibleOperator(ctx->op);
...
antlrcpp::Any TypeCheckVisitor::visitMaximum(AslParser::MaximumContext *ctx) {
  DEBUG_ENTER();
  for (auto ctxParam : ctx->expr()) visit(ctxParam);
  if (ctx->expr().size() < 2) {
    Errors.numberOfArguments(ctx->MAX());
    putTypeDecor(ctx,Types.createErrorTy());
  }
  else {
    TypesMgr::TypeId t1 = getTypeDecor(ctx->expr(0)); // Type first arg
    for (size_t i = 1; i < ctx->expr().size(); ++i) {
      TypesMgr::TypeId t = getTypeDecor(ctx->expr(i));
      if (!Types.isPrimitiveNonVoidTy(t) || !(Types.copyableTypes(t1,t) || Types.copyableTypes(t,t1))) {
        Errors.incompatibleArgument(ctx->MAX());
        t1 = Types.createErrorTy();
        break;
      }
      else if (Types.isFloatTy(t)) t1 = Types.createFloatTy();
    }
    putTypeDecor(ctx,t1);
  }
  putIsLValueDecor(ctx, false);
...
antlrcpp::Any TypeCheckVisitor::visitSumExpr(AslParser::SumExprContext *ctx) {
  ...
  for (auto ctxParam : ctx->expr()) {
    visit(ctxParam);
    t = getTypeDecor(ctxParam);
    if (!Types.isErrorTy(t) && !Types.isNumericTy(t)) {
      Errors.incompatibleOperator(ctx->SUM());
